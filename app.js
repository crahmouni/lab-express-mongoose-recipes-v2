...
app.get('/', (req, res) => {
    res.status(201).send("<h1>LAB | Express Mongoose Recipes</h1>");
});
...
app.post("/recipes", (req, res) => {
    ...
    .catch((error) => res.status(500).json({ message: "Error creating recipe", error }));
});
...
app.get("/recipes", (req, res) => {
    Recipe.find()
      ...
      .catch((error) => res.status(500).json({ message: "Error fetching recipes", error }));
  });
...
app.delete("/recipes/:id", (req, res) => {
  ...
  .catch((error) => res.status(500).json({ message: "Error deleting recipe", error }));
  });
